...
\begin{equation*}
  \mathcal P(p_1,p_2) = \setof{q}{X_{\#} q = p_1,X_{\#} = p_2}
    ...
    \begin{equation*}
      ...
      \end{cases}
    \end{equation*}
From the 1st and the 3dr equation, take $p(1,1) = \theta$ as a parameter, the parametrized solution is
\begin{equation*}
  \theta \mapsto p_\theta =
  \begin{bmatrix}
...
\bigskip
\paragraph{Supporting hyperplane} \emph{Let $A$ be an open convex set in $\reals^m$ and let $x$ be in the border of $A$. There exists a unit vector $w$ such that  $\scalarof w {y-x} < 0$ for all $y \in A$ that is, the half-space contains the convex set}.
...
Given a measure space $(\Omega,\mathcal A, \mu)$ a non-negative measurable function $p$ is a \emph{probability density} if $\int p \ d\mu = 1$. In such a case, the mapping
\begin{equation*}
  p \cdot \mu \colon \mathcal A \ni A \mapsto \int_A p \ d\mu = \int (A) p\ d\mu
\end{equation*}
is a probability measure.

In fact. $p \cdot \mu (\emptyset) = 0$ because $(\emptyset) = 0$, $p \cdot \mu(\Omega) = 1$ because $(\Omega) = 1$. If $(A_j)$ is a sequence of disjoint events, then $(\cup_j A_j) = \sum_j (A_j) = \sup_n \sum_{j=1}^n (A_j)$ and the result follows from additivity and monotone continuy.

The integral with respect to the push-forward of a probability measure given by a density is computed as follows. $X_{\#} \mu B = \mu(X^{-1}(B) = \int (X^{-1}(B)) \ d\mu = \int (B) \circ X d\mu$. It follows, by linearity, that for each non-negative simple function $f$ it holds $\int f \circ X d\mu = \int f d X_{\#}\mu$. By approximation from below the equality holds for all non-negative measurable function. In conclusion, the push-forward measure is characterised by the \emph{change of variable} formula
\begin{equation*}
...
\end{equation*}
Assume there exists a $\hat p$ such that the RHS equals $\int f \circ X \hat p \circ X \ d\mu$. Then, we can apply the chage of variable for $\mu$, and get
\begin{equation*}
  \int f \ d(X_{\#}p \cdot \mu) = \int f \hat p \ dX_{\#}\mu = \int f d \hat p \cdot X_{\#}\mu \ .
\end{equation*}
As $f$ is arbitrary, we have comuted the effect on the density of the push forward:
\begin{equation*}
  X_{\#}(p \cdot \mu) = \hat p \cdot X_{\#} \mu \quad \text{where} \forall f \in \mathcal L^+ \ \quad \int f \circ X p \ d\mu = \int f \circ X \hat p \circ X \ d\mu 
\end{equation*}

Now, a probability function $p$ is a density with respect to the counting measure. If $\Omega$ is finite, then the counting measure $\#(A) = \text{number of elements of $A$}$ is such that $\int_A p d\# = \sum_{\omega \in A} p(\omega)$. Consider now the push forward. In this case,
\begin{equation*}
  \int f \circ X p d\# = \sum_\omega f(X(\omega)) p(\omega) = \sum_{x \in \operatorname{Im}X} f(x) \sum_{x = X(\omega)} p(\omega) \ ,
\end{equation*}
