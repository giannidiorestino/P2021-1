...
\begin{equation*}
  \mu(F_n \geq c) \leq \euler^{-ac} \int \euler^{a F_n} \ d\mu =
  \euler^{-ac} \sum_{k=0}^n \euler^{ak/n} \binom n k 2^{-n} = 2^{-n}
  \euler^{-ac} \left(1 + \euler^{a/n}\right)^n \ .
\end{equation*}
Let us take the logarithm and minimize the RHS in $a > 0$.
\begin{equation*}
  \log \mu(F_n \geq c) \leq - \sup_{a > 0} \left(ac - n \log\frac{1+\euler^{a/n}}2\right)
\end{equation*}
