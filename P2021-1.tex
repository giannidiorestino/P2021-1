...
\begin{equation*}
  \int_\Omega \phi \circ X \ p \ d\mu = \int_S \phi \ dX_{\#} (p \cdot d\mu) \ .
\end{equation*}
...
Let $p$ be a probability function on $\Omega$, $X$ a non-constant real random variable, and $c >0$. For all $t > 0$,
\begin{equation*}
...
\emph{If one of the conditional probability functions is constant say, $P(y|x) = P(y)$, then $p_{XY}(x,y) = p_X(x) p_Y(y)$ and we say that $Y$ and $Y$ are independent}. In such a case, the other marginal probability function is constant. Equivalently, we can say that \emph{the distribution of $(X,Y)$ is a product probability measure.}

Let $Z = \phi(X,Y)$ be a real random variable in the space of random variable measurable with respect to $X$ and $T$, $L(X,Y)$. Then
...
\emph{If $X$ and $Y$ are independent, then the conditional expectation is constant and equal to the expectation.}
...
\begin{exercise}[Reversibility]
  ...
  \nu$. Moreover, $\diag \mu P = \diag \mu Q$, so that $P=Q$ when
  ...
  $\sum_c \alpha(c) \leq 1$. This provides a parametrization of the
  off-diagonal elements of of $\Gamma$. Then split the remaining mass
  on the diagonal.  \end{exercise}
